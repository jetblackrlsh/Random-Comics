...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Assemble an American Monomyth comic PDF from exactly 22 full-page images.",
    )
    parser.add_argument("--pages-dir", required=True, help="Directory containing cover, 20 story pages, and back cover.")
    parser.add_argument("--out", required=True, help="Output PDF path.")
    parser.add_argument("--preview-dir", default="tmp/american-monomyth-comic-book-pdf", help="Preview output directory.")
    ...
    if len(page_paths) != EXPECTED_PAGES:
        raise SystemExit(f"American Monomyth issues require exactly {EXPECTED_PAGES} images; found {len(page_paths)} in {pages_dir}")

    first_page = page_paths[0].name.lower()
    last_page = page_paths[-1].name.lower()
    if "cover" not in first_page:
        raise SystemExit(f"First sorted page must be the cover image; found {page_paths[0].name}")
    if "back-cover" not in last_page:
        raise SystemExit(f"Last sorted page must be the poster back cover image; found {page_paths[-1].name}")
